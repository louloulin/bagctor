...
import { PID, MessageMap } from '../types';
import { UserActor } from './user-actor';

// 用户Actor回复的消息类型
interface UserResponses extends MessageMap {
    'user.created': {
        user: {
            id: string;
            name: string;
...
describe('UserActor', () => {
    ...
    test('should list all users', async () => {
        ...
        expect(receivedMessages[0].payload.users.length).toBeGreaterThanOrEqual(2); // 前面创建了两个用户
    });
    ...
    test('should update a user', async () => {
        ...
        // 验证响应
        expect(receivedMessages.length).toBe(1);
        expect(receivedMessages[0].type).toBe('user.updated');
        expect(receivedMessages[0].payload.user).toBeDefined();
        expect(receivedMessages[0].payload.user.id).toBe(userId);
        expect(receivedMessages[0].payload.user.name).toBe('Robert Smith');
        expect(receivedMessages[0].payload.user.email).toBe('bob@example.com');
    });
    ...
    test('should delete a user', async () => {
        ...
        // 验证响应
        expect(receivedMessages.length).toBe(1);
        expect(receivedMessages[0].type).toBe('user.deleted');
        expect(receivedMessages[0].payload).toBeDefined();
        expect(receivedMessages[0].payload.id).toBe(userId);
        ...
        expect(receivedMessages.length).toBe(1);
        expect(receivedMessages[0].type).toBe('user.found');
        expect(receivedMessages[0].payload.user).toBeNull();
    });
}); 