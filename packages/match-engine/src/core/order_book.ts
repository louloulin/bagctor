...
export class OrderBook {
    ...
    while (oppositeOrders.length > 0 && order.filledQuantity.lessThan(order.quantity)) {
      ...
      for (let i = 0; i < bestLevel.orders.length; i++) {
        ...
        const tradeQuantity = Decimal.min?.(remainingQuantity, matchingRemaining) ||
          (remainingQuantity.lessThanOrEqualTo(matchingRemaining) ? remainingQuantity : matchingRemaining);
  ...
  // 获取当前买卖盘快照
  getSnapshot(): { bids: Array<[DecimalType, DecimalType]>; asks: Array<[DecimalType, DecimalType]> } {
    return {
