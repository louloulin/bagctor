...
export class ActorContext implements IActorContext, MessageInvoker {
  ...
  async spawn(props: Props): Promise<PID> {
    ...
    // 自动设置parent-child关系
    const childContext = this.system['contexts'].get(child.id);
    if (childContext) {
      childContext.setParent(this.pid);
      log.debug(`Established parent-child relationship: ${this.pid.id} -> ${child.id}`);
    } else {
      log.warn(`Failed to establish parent-child relationship: Child context not found for ${child.id}`);
    }
